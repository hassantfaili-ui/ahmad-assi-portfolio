---
title: River's View
sheet: A-110
category: Academic
year: 2022
location: Tunney's Pasture, Ottawa, Ontario
buildingType: Urban placemaking proposal
area: Site study
status: Unbuilt
role: With Awj, Exercise 3.2
contribution: Joint work with Awj. The concept, diagrams and analysis are shared.
summary: A placemaking proposal for a site at Tunney's Pasture, organised around the relationship between public and private ground on both axes.
leadImage:
  src: /media/tunney-02.jpg
  kind: site
  seed: 1
  alt: Parti and circulation diagrams for the River's View site, showing the relationship between public and private areas connecting north to south and east to west
imageGroups:
  - layout: pair
    caption: Analysis behind the proposal.
    images:
      - src: /media/tunney-03.jpg
        kind: site
        seed: 2
        alt: Analysis sheet from the urban placemaking review
      - src: /media/tunney-04.jpg
        kind: site
        seed: 3
        alt: Second analysis sheet from the urban placemaking review
  - layout: triptych
    caption: Remaining sheets from the review.
    images:
      - src: /media/tunney-05.jpg
        kind: site
        seed: 4
        alt: Further sheet from the urban placemaking review
      - src: /media/tunney-06.jpg
        kind: site
        seed: 5
        alt: Further sheet from the urban placemaking review
      - src: /media/tunney-01.jpg
        kind: site
        seed: 6
        alt: Cover sheet of the Exercise 3.2 urban placemaking review by Ahmad and Awj
drawings: []
featured: false
order: 10
---

The site sits at Tunney's Pasture, and the proposal is called River's View. The parti is a single relationship: how public ground meets private ground, and how that meeting can be made to connect the site north to south and east to west at the same time rather than choosing one axis.

The circulation diagram is where that gets tested. A parti that only works on paper falls apart as soon as you draw the routes people would actually take across it.

Joint work with Awj for Exercise 3.2.
